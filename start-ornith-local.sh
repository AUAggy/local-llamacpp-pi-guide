...
if lsof -nP -iTCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  if ps -axo command= | grep -F "llama-server" | grep -F "$MODEL" >/dev/null 2>&1; then
    echo "Ornith llama-server already appears to be running on $HOST:$PORT"
    exit 0
  fi

  echo "Port $PORT is already in use. Stop the current local model before starting Ornith." >&2
